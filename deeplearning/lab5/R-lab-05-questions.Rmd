---
title: "Deep Learning Lab 5 | Summer Term 2025"
...
You can find help on the "R interface to Keras" webpage: https://keras.posit.co/, and through the integrated help system of RStudio. Use it to learn about the Keras functions we will be using!

The networks that we will use are small enough that they can run on your personal computer, but, if you need a GPU, you can try the R Kernel for Jupyter on Google Colab (click [here](https://colab.research.google.com/github/IRkernel/IRkernel/blob/master/example-notebooks/Demo.ipynb)). At the beginning of your session, get a GPU by clicking on "Runtime", then "Change runtime type", then choose "GPU" as hardware accelerator. Then, install Keras for R using `install.packages` before loading it with `library(keras3)`. Note that the old `keras` package is deprecated and should not be used.
...
# Use a conda environment called if you have one or do above
reticulate::use_condaenv("retic")
...
# for convenience)
imdb <- keras3::dataset_imdb(num_words = 10000)
...
```{r}
dpt_model_early = (
  keras_model_sequential(input_shape = 10000) |>
...
library(ggplot2)
ggplot(contours) +
  geom_contour(aes(x, y, z = z, color = after_stat(level))) +
  theme_minimal() +
...
plot_histories = function(histories) {
  ...
  ggplot() +
    ...
    geom_line(aes(histories$x, histories$y, 
                  color = histories$name, linetype = histories$name)) +
