---
title: "Deep Learning Lab 7 | Summer Term 2025"
...
if(!file.exists(dst)) {
  ...
  options(timeout=360)
  download.file(url, dst, method = "curl")
}
...
Let's first create datasets from the directories we have just created:

```{r}
library(keras3)
data_source_train = image_dataset_from_directory(
...
data_source_validation = image_dataset_from_directory(
...
The augmentation steps i.e. layers are added to the model definition. Those layers will only act during training, and will be disabled during validation and testing. Why so? For a reference of possible augmentation layers, please refer to the [documentation](https://keras.posit.co/reference/index.html#image-augmentation-layers).
...
model = keras_model_sequential(input_shape = c(224, 224, 3)) |>
  ...
  layer_random_rotation(factor=0.3) |>
  layer_random_brightness(0.5, value_range = c(0.0, 1.0)) |>
  layer_random_zoom(0.2, 0.2) |>
...
# summary(vgg_pretrained) will give you now also an overview of whether the layer is trainable
```
...
**(b)** Compute the receptive field size of the pre-trained VGG16 architecture we used above, right before the global average pooling layer.

**(c)** Now suppose to have a dilation of $d_\ell\geq 1$ at every layer. What is the new formula for $r_0$?

**(d)** What is the most effective way to increase the size of the receptive field of a neural network?
...
**(b)** The pre-trained VGG16 has five blocks composed by two (first two blocks) or three (last three blocks) convolutions with kernel size 3 and stride 1 followed by a max pooling of kernel size 2 with stride 2. We can easily apply the recursive formula:
