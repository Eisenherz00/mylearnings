...
for(i in 1:max_len) {
  ...
  # TODO apply the encoder to the i-th digit of the first number
}
second_number_encoded <- list()
for(i in 1:max_len) {
  # TODO apply the encoder to the i-th digit of the second number
}
```
...
encoded_numbers <- layer_concatenate(
  # TODO concatenat and reshape appropriately
)
```
...
# TODO add the multi-head attention layer.

# TODO add the global average pooling layer
